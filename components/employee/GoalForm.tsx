...
export function GoalForm({
  ...
  const [form, setForm] = useState({
    ...
    target:
      initialData?.target !== undefined && initialData?.target !== null
    ...
    deadline: initialData?.deadline
      ...
      : "",
    weightage:
      initialData?.weightage !== undefined
  ...
  function validate() {
    ...
    if (form.uomType === "TIMELINE") {
      if (!form.deadline) {
        e.deadline = "Deadline date is required"
      }
    } else {
      if (form.target === "" || Number.isNaN(Number(form.target))) {
        e.target = "Enter a valid number"
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        {errors.thrustArea && (
          <p className="text-xs text-red-500">{errors.thrustArea}</p>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        <Label>
          Unit of Measurement Type <span className="text-red-500">*</span>
        </Label>
        <Select
          value={form.uomType}
          disabled={isSharedGoal}
          onValueChange={(v) =>
            setForm({
              ...form,
              uomType: v ?? "",
              target: v === "ZERO" ? "0" : v === "TIMELINE" ? "" : form.target,
              deadline: v === "TIMELINE" ? form.deadline : "",
            })
          }
        >
        ...
        {errors.uomType && (
          <p className="text-xs text-red-500">{errors.uomType}</p>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        <Label>
          {uomInfo?.targetLabel || "Target Value"}{" "}
          ...
          <>
            ...
            {form.uomType === "ZERO" && (
              <p className="text-xs text-slate-400">
                Target is fixed at 0 for ZERO type goals
              </p>
            )}
            {errors.target && (
      ...
      <div className="space-y-1.5">
        ...
        />
        {errors.weightage && (
          <p className="text-xs text-red-500">{errors.weightage}</p>
        )}
      </div>
