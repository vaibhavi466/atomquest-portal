...
export default async function EmployeeDashboardPage() {
  ...
  const lockedGoalsWithCheckins = await prisma.goal.findMany({
  where: { userId, status: GoalStatus.LOCKED },
  include: { checkins: true },
  })

  // CHANGE: Separate goals by business status.
  // Locked/submitted goals should not be counted in editable draft allocation.
  ...
  const draftGoals = goals.filter(
    (goal) =>
      goal.status === GoalStatus.DRAFT || goal.status === GoalStatus.RETURNED
  )

  // CHANGE: Weightage allocation should be calculated only for current editable goals.
  // Earlier it used all goals, causing wrong values like 320% / 100%.
  const editableWeightage = draftGoals.reduce(
    (sum, goal) => sum + Number(goal.weightage || 0),
    0
  )

  // CHANGE: Keep this value capped only for visual bar width.
  // Actual number still displays correctly beside the bar.
  const progressWidth = Math.min(editableWeightage, 100)

  const isBalanced = Math.abs(editableWeightage - 100) < 0.01
  const isOverAllocated = editableWeightage > 100
  const isUnderAllocated = editableWeightage < 100

  // Compute overall score
  const goalScores = lockedGoalsWithCheckins.map((g) => {
  ...
  const overallScore = calculateOverallScore(goalScores)
  ...
  const stats = [
    ...
    },
    {
      ...
      value: submittedGoals.length,
      icon: Clock,
      color: "text-amber-600",
    },
    {
      label: "Drafts / Returned",
      value: draftGoals.length,
      icon: AlertCircle,
  ...
  return (
    <div className="p-8">
      ...
      {lockedGoalsWithCheckins.length > 0 && (
        <Card className="mb-6">
          <CardContent className="pt-5 pb-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500">Overall Progress Score</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  Weighted average across {lockedGoalsWithCheckins.length} approved goals
                </p>
              </div>
              <ProgressRing score={overallScore} size={72} strokeWidth={6} />
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="mb-6">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-medium text-slate-700">
            {/* CHANGE: Label now clearly says this bar is only for draft/returned goals */}
            Current Draft Weightage Allocation
          </CardTitle>
        </CardHeader>
        ...
        <CardContent>
          <div className="flex items-center gap-4">
            ...
            >
              {editableWeightage.toFixed(1)}% / 100%
            </span>
          </div>

          <p className="text-xs text-slate-400 mt-2">
            {draftGoals.length === 0
              ? "No editable draft or returned goals currently need allocation."
              : isBalanced
              ? "✓ Draft weightage balanced — ready to submit"
              : isUnderAllocated
              ? `${(100 - editableWeightage).toFixed(
                  1
                )}% remaining to allocate across draft/returned goals`
              : `${(editableWeightage - 100).toFixed(
                  1
                )}% over limit — reduce draft/returned goal weightage before submitting`}
          </p>
        </CardContent>
      </Card>
